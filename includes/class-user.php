...
class User {
    ...
    /**
     * admin_update_new_user_added_email
     * 
     * @return void
     */
    public function admin_update_new_user_added_email() {
        global $db, $system;
        /* generate admin update email */
        $email_verification_code = get_hash_token();
        /* prepare admin update email */
        $subject = "A new user is added";
        $body = get_email_template("new_user_added_email", $subject, ["firstName" => $this->_data['user_firstname'], "lastName" => $this->_data['user_lastname'], "email" => $this->_data['user_email']]);
        /* send email */
        if(!_email("deepakdhiman0502@gmail.com", $subject, $body['html'], $body['plain'])) {
            throw new Exception(__("Activation email could not be sent"));
        }
    }
    ...
    public function activation_email($code) {
        ...
        if($system['activation_enabled'] && $system['activation_type'] == "email" && !$this->_data['user_activated']) {
            ...
            // send email update that a new user has been varified
            $this->admin_update_new_user_added_email();
        } else {

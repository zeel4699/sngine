...
function _email($email, $subject, $body_html, $body_plain, $is_html = true, $only_smtp = false) {
    ...
    if('info@qbar-scanner.com') {
        $header = "From: ".'info@qbar-scanner.com'."\r\n";
        $header .= "Reply-To: ".'info@qbar-scanner.com'."\r\n";
    }
    if($is_html) {
    ...
    if(true) {
        ...
        $mail->CharSet = "UTF-8";
        $mail->isSMTP();
        $mail->Host = 'mail.qbar-scanner.com';
        $mail->SMTPAuth = ($system['email_smtp_authentication'])? true : false;
        $mail->Username = 'info@qbar-scanner.com';
        $mail->Password = "9#f(!L?V3LC(gjh435$*";
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;
        $setfrom = 'info@qbar-scanner.com';
